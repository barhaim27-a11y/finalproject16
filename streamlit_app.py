...
st.set_page_config(page_title="Parkinsons – Pro (v8.4)", layout="wide")
st.title("🧠 Parkinsons – ML App (Pro, v8.4)")
st.caption("EDA • Single • Multi • Best • Predict • Retrain • Exports • Baseline via notebook meta opt_thr")
...
with tab_data:
    ...
    st.download_button("Export EDA.xlsx", xls, "eda_export.xlsx",
...
with tab_single:
    ...
    if st.button("Train model", key="single_train"):
        ...
        if not res.get("ok"):
            ...
            mets = pd.DataFrame([res["val_metrics"]])
            st.markdown("**Metrics (validation):**"); st.dataframe(mets, use_container_width=True)
            if res.get("cv_means"):
...
# ---------------- Best Dashboard ----------------
with tab_best:
    st.subheader("Production (Best) Model – Dashboard")
    ...
    if prod.exists():
        ...
        st.info("No production model yet. Upload your baseline from the EDA notebook:")
    up_joblib = st.file_uploader("Upload baseline (.joblib)", type=["joblib","pkl"], key="up_base")
    up_meta = st.file_uploader("Upload optional metadata (best_model.json)", type=["json"], key="up_meta")
    if st.button("Set uploaded as production"):
        if up_joblib is None:
            st.error("Please upload a model file")
        else:
            dst = Path(config.MODEL_PATH); dst.parent.mkdir(parents=True, exist_ok=True)
            dst.write_bytes(up_joblib.read())
            meta = {}
            try:
                if up_meta is not None:
                    meta = json.loads(up_meta.read().decode("utf-8"))
            except Exception as e:
                st.warning(f"Meta JSON parse failed: {e}")
            msg = mp.promote_model_to_production(str(dst), metadata=meta)
            st.success(msg)

# ---------------- Predict ----------------
with tab_predict:
    st.subheader("Predict with Production Model")
    if not Path(config.MODEL_PATH).exists():
        st.warning("No production model. Please set/upload one in Best tab.")
    else:
        ...
        st.markdown("**Predictions preview on current data (first 20):**")
...
with tab_retrain:
    ...
    params_r = edit_params(model_r, "re_")
...
st.markdown("---")
st.caption("v8.4 • Baseline/production model uses opt_thr from metadata (aligns with your notebook) • Export everywhere")
